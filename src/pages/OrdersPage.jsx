...
const OrdersPage = () => {
  const { getOrdersByUser, getPaymentByOrderId, uploadPaymentScreenshot, updateOrderStatus } = useOrder();
  const { user } = useAuth();
  ...
  const [returnReasonSelect, setReturnReasonSelect] = useState('Size not correct');
  ...
  const handleSubmitReturnRequest = async () => {
    ...
    const finalReason = returnReasonSelect === 'Other' ? customReturnReason.trim() : returnReasonSelect;
    const success = await updateOrderStatus(returnOrder.id, 'return_pending', null, { customerReturnReason: finalReason });
    if (success) {
          ...
          {orders.map(order => {
            ...
            return (
              <div key={order.id} className={`card p-5 animate-fade-in ${isNew ? 'ring-2 ring-success' : ''}`}>
                ...
                <div className="flex items-start justify-between gap-4 flex-wrap">
                  ...
                  <div className="flex items-center gap-2">
                    <div className={`badge gap-1 ${
                      order.status === 'delivered' || order.status === 'return_approved' ? 'badge-success' :
                      order.status === 'payment_rejected' || order.status === 'return_rejected' ? 'badge-danger' :
                    ...
                    {order.paymentMethod === 'gpay' ? (
                      <>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          {order.status === 'delivered' && (
                            <button
                              onClick={() => { setReturnOrder(order); setReturnReasonSelect('Size not correct'); }}
                              className="text-xs text-accent-700 hover:text-accent-800 font-semibold underline"
                            >
                              Request Return
                            </button>
                ...
                {/* Tracking Timeline / Return Status */}
                {selectedOrder?.id === order.id && (
                  <div className="mt-4 pt-4 border-t border-neutral-100 animate-fade-in">
                    {order.status === 'payment_rejected' ? (
                      <div className="bg-danger/10 border border-danger/20 rounded-xl p-3 text-xs text-danger font-medium">
                        Payment verification failed. Please check with support or place a new order.
                      </div>
                    ) : order.status === 'return_pending' ? (
                      <div className="bg-warning/10 border border-warning/20 rounded-xl p-3 text-xs text-primary font-medium">
                        <p className="font-semibold text-warning mb-1">Return Request Under Review</p>
                        <p><strong>Reason:</strong> {order.customerReturnReason}</p>
                      </div>
                    ) : order.status === 'return_approved' ? (
                      <div className="bg-success/10 border border-success/20 rounded-xl p-3 text-xs text-success font-medium">
                        <p className="font-semibold mb-1">Return Approved ✓</p>
                        <p className="text-neutral-600">Your return request has been approved. Our team will contact you shortly regarding the pickup.</p>
                        {order.customerReturnReason && (
                          <p className="text-neutral-500 mt-1"><strong>Reason for Return:</strong> {order.customerReturnReason}</p>
                        )}
                      </div>
                    ) : order.status === 'return_rejected' ? (
                      <div className="bg-danger/10 border border-danger/20 rounded-xl p-3 text-xs text-danger font-medium">
                        <p className="font-semibold mb-1">Return Request Rejected ✗</p>
                        <p className="text-neutral-700"><strong>Reason for Rejection:</strong> {order.returnReason || 'No reason provided.'}</p>
                        {order.customerReturnReason && (
                          <p className="text-neutral-500 mt-1"><strong>Your Return Reason:</strong> {order.customerReturnReason}</p>
                        )}
                      </div>
                    ) : (
                      <>
        ...
        <div className="bg-white rounded-2xl w-full max-w-md p-6 animate-scale-in max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
          ...
          <div className="space-y-2 mb-4">
            {[
              ...
              <label key={reason} className="flex items-center gap-2 cursor-pointer py-1">
                <input 
                  type="radio" 
                  name="return_reason" 
                  checked={returnReasonSelect === reason}
                  onChange={() => setReturnReasonSelect(reason)}
          ...
          <div className="flex gap-2 justify-end pt-2 border-t border-neutral-100">
            <button 
              onClick={() => { setReturnOrder(null); setCustomReturnReason(''); }}
            ...
            <button 
              ...
              disabled={returnReasonSelect === 'Other' && !customReturnReason.trim()}
            >
              Submit Request
            </button>
          </div>
